...
- OpenAI-compatible public config/builder/runtime surfaces now also expose an explicit provider-level `supportsStructuredOutputs` policy: callers can request AI SDK-style downgrade of JSON Schema chat outputs to `response_format = { "type": "json_object" }`, and explicit opt-out now emits a stable `unsupported { feature: "responseFormat" }` warning on chat responses. The default remains permissive for backward compatibility and is documented in the workstream notes as the remaining divergence from AI SDK's conservative default.
