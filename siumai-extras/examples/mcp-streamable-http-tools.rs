//! MCP streamable HTTP tool discovery example.
//!
//! Run:
//! ```bash
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let url = std::env::var("MCP_URL").unwrap_or_else(|_| "http://localhost:3000/mcp".to_string());
    let (tools, resolver) = mcp_tools_from_http(&url).await?;
