...
use super::{client::CohereClient, config::CohereConfig};
...
impl CohereBuilder {
    ...
    pub fn build(self) -> Result<CohereClient, LlmError> {
        ...
        let mut client = if let Some(http_client) = http_client_override {
            CohereClient::with_http_client(config, http_client)?
        } else {
            CohereClient::from_config(config)?
        };
...
mod config_first_tests {
    ...
    fn into_config_preserves_explicit_api_key_and_http_interceptors() {
        let cfg = CohereBuilder::new(BuilderBase::default())
            .api_key("test-key")
            .model("rerank-v3.5")
        ...
        assert_eq!(cfg.common_params.model, "rerank-v3.5");
        assert_eq!(
    ...
    fn into_config_matches_manual_config_for_http_conveniences() {
        let builder_cfg = CohereBuilder::new(BuilderBase::default())
            .api_key("test-key")
            .model("rerank-v3.5")
            .timeout(std::time::Duration::from_secs(15))
            .connect_timeout(std::time::Duration::from_secs(3))
        ...
        let manual_cfg = CohereConfig::new("test-key")
    ...
    fn build_and_into_config_converge_on_config_first_client_construction() {
        let builder = CohereBuilder::new(BuilderBase::default())
            .api_key("test-key")
            .model("rerank-english-v3.0");

        let cfg = builder.clone().into_config().expect("into_config");
        let built = builder.build().expect("build client");
        let from_config = CohereClient::from_config(cfg).expect("from_config client");

        assert_eq!(crate::client::LlmClient::provider_id(&built), "cohere");
        assert_eq!(
