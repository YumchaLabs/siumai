...
fn extract_raw_message(err: &LlmError) -> String {
    match err {
        ...
        LlmError::NoImageGenerated { .. } => "No image generated.".to_string(),
        LlmError::NoSpeechGenerated { .. } => "No speech audio generated.".to_string(),
        LlmError::NoTranscriptGenerated { .. } => "No transcript generated.".to_string(),
...
/// Best-effort provider hint by model id.
///
/// This function is intentionally lightweight and does not consult any global
/// registries. It only recognizes the `provider:model` convention used by
/// the registry handle APIs.
pub fn provider_hint_for_model(model: Option<&str>) -> Option<ProviderHint> {
    ...
    Some(ProviderHint {
        suggested_provider_id: Some(provider_id.to_string()),
        aliases: Vec::new(),
    })
}
