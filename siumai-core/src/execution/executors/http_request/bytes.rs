...
use crate::execution::executors::helpers::{
    apply_before_send_interceptors, build_multipart_body, rebuild_headers_and_retry_once,
    with_multipart_content_headers,
};
...
pub async fn execute_bytes_request(
    ...
    if let Some(transport) = &config.transport {
        ...
        return Ok(crate::execution::executors::common::HttpBytesResult {
            bytes: result.body,
    ...
    F: Fn() -> Result<reqwest::multipart::Form, LlmError>,
{
    ...
    let rb2 = apply_before_send_interceptors(
        &config.interceptors,
        &ctx,
        rb,
        &empty_json,
