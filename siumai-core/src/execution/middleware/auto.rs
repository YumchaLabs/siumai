...
pub fn build_auto_middlewares(config: &MiddlewareConfig) -> MiddlewareBuilder {
    ...
    // 1. Add default middlewares (provider-agnostic)
    add_default_middlewares(&mut builder, config);
...
/// Add default middlewares (provider-agnostic).
///
/// Core should avoid embedding provider-specific branching logic. If a provider
/// needs special-case behavior, prefer:
/// - protocol transformers in `standards/*`, or
/// - explicit provider extension APIs in provider crates, or
/// - registry wiring that injects provider/model middlewares externally.
fn add_default_middlewares(builder: &mut MiddlewareBuilder, config: &MiddlewareConfig) {
