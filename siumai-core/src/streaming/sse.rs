...
mod tests {
    ...
    #[tokio::test]
    async fn test_sse_stream_keep_alive_comments_do_not_break_parsing() {
        // Some providers emit SSE keep-alive comments like `: ping`.
        // We don't require the parser to surface them as events; we only require that
        // subsequent data events are still parsed correctly.
        let data: Vec<Result<&[u8], std::io::Error>> = vec![
            Ok(b": keep-alive\n\n".as_slice()),
            Ok(b"data: hello\n\n".as_slice()),
            Ok(b"\n".as_slice()),
        ...
        while let Some(item) = sse_stream.next().await {
            ...
            if !event.data.trim().is_empty() {
                non_empty.push(event.data);
            }
        }

        assert_eq!(non_empty, vec!["hello".to_string(), "world".to_string()]);
    }
}
