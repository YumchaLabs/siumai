...
- OpenAI-compatible request settings now also include AI SDK-style `includeUsage` / `transformRequestBody` concepts: compat chat streams omit `stream_options.include_usage` unless the provider config opts in explicitly, `OpenAiCompatibleRequestSettings` carries that provider-level policy into the runtime spec layer, and callers can install a public `RequestBodyTransformer` hook that runs after provider normalization on the final request body.
