...
- The compat request-settings lane now also exposes an explicit provider-level `supportsStructuredOutputs` policy for chat-completions shaping: when callers set it to `false`, JSON Schema `response_format` requests are downgraded to `json_object` before transport, while leaving the historical permissive default untouched for backward compatibility.
