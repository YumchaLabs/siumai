...
mod tests {
    ...
    fn test_builder_creation() {
        ...
        #[cfg(feature = "openai")]
        {
            let _openai_builder = LlmBuilder::new().openai();
            let _ = _openai_builder;
        }
    }
    ...
    fn test_http_config_inheritance() {
        ...
        // Always assert base builder config is stored correctly.
        assert_eq!(base_builder.timeout, Some(Duration::from_secs(60)));
        assert_eq!(
            base_builder.proxy,
            Some("http://proxy.example.com:8080".to_string())
        );
        assert_eq!(base_builder.user_agent, Some("test-agent/1.0".to_string()));
        assert!(base_builder.default_headers.contains_key("X-Test-Header"));
