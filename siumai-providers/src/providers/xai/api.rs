...
impl XaiModels {
    ...
    async fn list_models_from_api(&self) -> Result<Vec<ModelInfo>, LlmError> {
        ...
        let spec = Arc::new(super::spec::XaiSpec);
        ...
        let xai_response: XaiModelsResponse = serde_json::from_value(result.json).map_err(|e| {
            LlmError::ParseError(format!("Failed to parse xAI models response: {e}"))
        })?;
    ...
    async fn get_model_from_api(&self, model_id: String) -> Result<ModelInfo, LlmError> {
        ...
        let result =
            crate::execution::executors::common::execute_get_request(&config, &url, None).await?;
        let xai_model: XaiModel = serde_json::from_value(result.json).map_err(|e| {
            LlmError::ParseError(format!("Failed to parse xAI model response: {e}"))
        })?;
