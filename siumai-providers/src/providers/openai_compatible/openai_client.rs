...
impl OpenAiCompatibleClient {
    ...
    fn build_embedding_executor(&self, request: &EmbeddingRequest) -> Arc<HttpEmbeddingExecutor> {
        ...
        EmbeddingExecutorBuilder::new(self.config.provider_id.clone(), self.http_client.clone())
            .with_spec(spec)
            .with_context(ctx)
            .with_interceptors(self.http_interceptors.clone())
        ...
        if let Some(opts) = &self.retry_options {
            ...
            crate::retry_api::retry_with(
                || {
                    ...
                    let exec = exec.clone();
                    async move { ChatExecutor::execute(&*exec, reqc).await }
                },
                opts,
            )
            .await
        ...
        if let Some(opts) = &self.retry_options {
            ...
            crate::retry_api::retry_with(
                || {
                    let reqc = request.clone();
                    let exec = exec.clone();
                    async move { ChatExecutor::execute_stream(&*exec, reqc).await }
                },
                opts,
            )
            .await
        ...
        if let Some(opts) = &self.retry_options {
            ...
            crate::retry_api::retry_with(
                || {
                    let rqc = req.clone();
                    let exec = exec.clone();
                    async move { EmbeddingExecutor::execute(&*exec, rqc).await }
                },
                opts,
            )
            .await
...
impl OpenAiCompatibleClient {
    ...
    async fn get_model_internal(&self, model_id: String) -> Result<ModelInfo, LlmError> {
        ...
        let direct = crate::execution::executors::common::execute_get_request(&config, &url, None)
            .await;
        ...
        if let Some(opts) = &self.retry_options {
            crate::retry_api::retry_with(
                || {
                    let rqc = request.clone();
                    let exec = exec.clone();
                    async move { ImageExecutor::execute(&*exec, rqc).await }
                },
                opts.clone(),
            )
            .await
...
mod tests {
    ...
    async fn build_chat_executor_wires_before_send_for_custom_options_with_runtime_provider_id() {
        ...
        custom.insert("my_custom".to_string(), serde_json::json!(1));
