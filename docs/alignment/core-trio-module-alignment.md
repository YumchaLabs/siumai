...
- **Streaming semantics**:
  ...
  - `functionResponse` is naturally _next-request input_ in Gemini; for gateways we support replay as a stream frame (opt-in).
...
- Multi-target SSE gateway helpers: `siumai-extras/src/server/axum/mod.rs`
