...
- [x] Record the bounded AI SDK root-export audit.
  ...
  - `createProviderDefinedToolFactory`,
    ...
    `ProviderDefinedTool` carrier. The passive provider-tool shape now preserves
    `type: "provider"`, `isProviderExecuted`, `inputSchema`, `outputSchema`, `args`, and
    `supportsDeferredResults`, while local execution remains bound through `ExecutableTool`.
