...
4. the OpenAI-compatible provider-settings surface is closer but still not complete:
   `metadataExtractor`, `includeUsage`, and `transformRequestBody` now exist on the public
   config/builder/runtime path, but `queryParams` and a cleaner `supportsStructuredOutputs`
   provider-level policy still need a dedicated follow-up pass
