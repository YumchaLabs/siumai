...
3. request-boundary compatibility cleanup is now bounded, but a few deliberately retained shims
   still need a final remove-or-deprecate pass once coverage is considered sufficient
4. shared non-chat request abstractions are improved but not fully AI SDK-complete:
   `ImageEditRequest` now models typed multi-input `images[]` + `mask` semantics, but URL-backed
   edit inputs are still provider-conditional on multipart/inline paths, and the shared video
   request shape now has canonical typed inputs/knobs but still carries some provider-centric
   fields that should likely be pushed behind provider-owned typed layers
...
| Provider-owned image/video requests | `packages/xai/src/xai-image-model.ts`, `packages/xai/src/xai-video-model.ts`, `packages/xai/src/xai-image-options.ts`, `packages/xai/src/xai-video-options.ts`, `packages/provider/src/video-model/v4/video-model-v4-call-options.ts`, `packages/provider/src/video-model/v4/video-model-v4-file.ts` | `siumai-spec/src/types/image.rs`, `siumai-spec/src/types/video.rs`, `siumai-provider-xai/src/providers/xai/image.rs`, `siumai-provider-xai/src/providers/xai/video.rs`, `siumai-provider-gemini/src/providers/gemini/video.rs`, `siumai-provider-minimaxi/src/providers/minimaxi/video.rs` | Amber | The provider-owned image/video routing is now much closer to the audited AI SDK shape on both the shared and provider boundaries: typed `XaiImageOptions` / `XaiVideoOptions` plus request ext traits are public, image generation/edit map to `/images/generations` and `/images/edits`, xAI video create/query map to `/videos/generations|edits` and `GET /videos/{request_id}`, shared `ImageEditRequest` models typed multi-input `images[]` + `mask`, and shared `VideoGenerationRequest` now carries typed `VideoGenerationInput` file/url inputs plus canonical `count` (`n`), `fps`, and `seed`. xAI now warns-and-filters unsupported `n` / `fps` / `seed`, Gemini/Veo consumes canonical `count` / `seed` / typed image input and surfaces warnings for unsupported URL/FPS cases, and MiniMaxi now filters unsupported shared video fields instead of serializing the whole request object blindly. Remaining gaps are now narrower and explicitly shared/provider-boundary ones: URL-backed image edit inputs still need a shared async materialization layer on multipart/inline paths, and `VideoGenerationRequest` still carries MiniMaxi-centric top-level fields that should likely move behind provider-owned typed layers. |
...
2. continue tightening public-path fixture coverage for the remaining OpenAI-compatible raw-hint edges beyond the now-covered metadata/logprobs/prediction-token paths
3. request-boundary compatibility cleanup outside the already-audited provider/protocol paths
4. final cleanup of the remaining deliberately retained request-boundary compatibility shims
...
### Cut 3 - Continue request-boundary compatibility cleanup

Target result:

- request-only behavior reads `providerOptions` first across the remaining unaudited paths
- legacy `provider_metadata` / `metadata.custom` request fallbacks become explicitly bounded
