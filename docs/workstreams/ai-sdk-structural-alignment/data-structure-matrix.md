...
4. the OpenAI-compatible provider-settings surface is now materially closer to AI SDK:
   ...
   path now, but the default `supportsStructuredOutputs` behavior still intentionally stays
   permissive for backward compatibility instead of matching AI SDK's conservative default `false`
