...
3. shared non-chat request abstractions are improved but not fully AI SDK-complete:
   `ImageEditRequest` now models typed multi-input `images[]` + `mask` semantics, compat image
   request shaping now reads provider-owned options from deprecated `openai-compatible`, canonical
   `openaiCompatible`, and provider-owned keys while surfacing AI SDK-style `seed` warnings, but
   URL-backed edit inputs are still provider-conditional on multipart/inline paths, the shared
   image request family still does not expose top-level `aspectRatio`, edit/variation requests
   still do not expose a shared `seed` knob, and the shared video request shape now has canonical
   typed inputs/knobs but still carries some provider-centric fields that should likely be pushed
...
| Shared image request surface | `packages/provider/src/image-model/v4/image-model-v4-call-options.ts` | `siumai-spec/src/types/image.rs`, `siumai-protocol-openai/src/standards/openai/image.rs`, `siumai-protocol-openai/src/standards/openai/compat/spec.rs` | Amber | OpenAI-compatible image request shaping now follows the AI SDK provider-options lane more closely: generation/edit/variation merge provider-owned options from deprecated `openai-compatible`, canonical `openaiCompatible`, and provider-owned keys instead of hardcoding `providerOptions.openai|azure`, and image generation now surfaces AI SDK-style `unsupported { feature: "seed" }` warnings instead of silently dropping `seed`. Remaining structural gaps: the shared Rust image request family still does not expose top-level `aspectRatio`, and split edit/variation requests still lack a shared `seed` knob comparable to AI SDK `ImageModelV4CallOptions`. |
