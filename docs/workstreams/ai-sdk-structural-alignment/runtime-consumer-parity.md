...
- does it avoid falling back to provider-specific custom payloads before typed parts
- does it avoid double-applying tool arguments when replay metadata and semantic parts appear together
...
### 1. `stream_object` now consumes typed text/tool parts directly
...
### 2. The extras tool loop now accumulates typed tool-call state
...
- locally executed tool results now also re-enter the downstream stream as stable
  `Part(ToolResult)` before the `gateway:tool-result` compatibility custom event

Why this matters:

- it closes the last high-level consumer bug where protocol parsers could be correct but the tool
  loop would still behave as if provider-specific custom payloads were authoritative
...
- it removes another hidden dependency on text shadow emission from provider parsers or gateway
  bridges
...
- shared `StreamFactory` content-presence detection now treats stable `Part(TextDelta)` /
  `PartWithReplay(TextDelta)` as real text
- `SimulateStreamingMiddleware` now also treats stable text parts as existing text before deciding
  whether it should synthesize fallback text from `StreamEnd.response`

Why this matters:

- semantic-only streams no longer pick up an extra tail copy of the final text
- fallback synthesis stays behind the stable semantic lane
...
- the main streaming examples, migration snippets, gateway transform example, and bridge/transcode
  tests now consume text through `event.text_delta()` or stable `ChatStreamPart::TextDelta`
- gateway stream-hook guidance now explicitly tells callers to mutate stable text/reasoning parts,
  not provider-specific shadow payloads
...
- the custom-provider example now emits stable `ChatStreamEvent::Part(ChatStreamPart::TextDelta)`
  directly instead of teaching new provider authors to synthesize provider-specific shadow text
...
- `delta` / `tool` / `reasoning` / provider `custom` frames remain compatibility output,
  not the preferred semantic export lane

### 2. The runtime naming story is still heavier than AI SDK
