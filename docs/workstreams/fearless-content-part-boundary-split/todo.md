...
- [ ] Pick one low-risk request construction path that still emits legacy `ContentPart` directly.
- [ ] Route it through a named request-side adapter that guarantees no response metadata is read.
