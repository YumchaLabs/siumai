...
- Azure, DeepSeek, Vertex MaaS, Ollama, XAI, Bedrock, Anthropic, Groq, MiniMaxi, and Google Vertex
  public-path registry setup use `RegistryBuilder` provider-level shortcuts rather than
  hand-written `RegistryOptions { provider_build_overrides: ... }` maps.
