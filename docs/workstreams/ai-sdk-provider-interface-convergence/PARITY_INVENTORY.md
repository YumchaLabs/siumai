...
| --- | --- | --- | --- | --- |
| `openai` | `siumai-provider-openai`, `siumai-protocol-openai`, `provider_ext::openai` | AI SDK-aligned native OpenAI plus Responses/Chat/Completion/Embedding/Files/Skills typed surfaces | Green | Responses provider metadata envelope names from the package index are exported through the OpenAI provider/facade; stream/custom-tool replay is covered by stable-part protocol and bridge gates while provider-custom replay remains an explicit compatibility boundary. |
