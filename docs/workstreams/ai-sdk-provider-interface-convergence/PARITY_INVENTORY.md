...
| Stream part union | `language-model-v4-stream-part.ts` | `ChatStreamEvent::Part`, `ChatStreamPart`, `LanguageModelV4StreamPart`, replay carriers | Amber | OpenAI Responses and gateway tool assertions now use stable `Part` tests; Anthropic/Gemini custom serializer inputs are guarded as V3/provider-native compatibility only. |
