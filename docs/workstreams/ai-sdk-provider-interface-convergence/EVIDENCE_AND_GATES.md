...
| 2026-05-18 | `cargo fmt -p siumai-provider-openai-compatible -p siumai-registry -p siumai-spec -p siumai -- --check` | Passed | Formatting gate for the Google Vertex xAI package-boundary slice. |
| 2026-05-18 | `cargo nextest run -p siumai-provider-openai-compatible --all-features google_vertex_xai_request_transformer_strips_reasoning_knobs google_vertex_xai --no-fail-fast` | Passed | 2 focused provider tests passed, proving Google Vertex xAI strips shared reasoning knobs and preserves package model/modeling behavior. |
| 2026-05-18 | `cargo nextest run -p siumai-registry --features google-vertex provider_catalog_uses_native_metadata_for_google_vertex_xai google_vertex_xai --no-fail-fast` | Passed | 2 registry tests passed, proving Google Vertex xAI catalog metadata and builder methods stay aligned. |
| 2026-05-18 | `cargo nextest run -p siumai --features google-vertex public_surface_google_vertex_xai_provider_ext_compiles --test public_surface_imports_test --no-fail-fast` | Passed | Public-surface compile guard passed for `provider_ext::google_vertex_xai::{google_vertex_xai, create_google_vertex_xai}` and `Provider::google_vertex_xai()` / `Provider::vertex_xai()`. |
| 2026-05-18 | `cargo nextest run -p siumai --features google-vertex google_vertex_xai --test provider_public_path_parity_test --no-fail-fast` | Passed | 5 parity tests passed, proving Google Vertex xAI package, facade, config, and registry request paths are equivalent and reasoning knobs are normalized away. |
| 2026-05-18 | `cargo fmt -p siumai-spec -p siumai -- --check` | Passed | Formatting gate for the Google Vertex xAI spec/facade follow-up. |
| 2026-05-18 | `cargo nextest run -p siumai-spec provider_type_maps_google_vertex_xai_name --no-fail-fast` | Passed | Provider type mapping guard passed for `google-vertex-xai` aliases. |
| 2026-05-18 | `cargo nextest run -p siumai --features openai,azure,anthropic,google,xai,ollama,groq,deepseek test_provider_type_consistency --test siumai_unified_interface_test --no-fail-fast` | Passed | Unified-interface provider type consistency guard passed after adding Azure to the actual test feature set and covering `google-vertex-xai`. |
| 2026-05-18 | `cargo fmt -p siumai-provider-azure -p siumai -- --check` | Passed | Formatting gate for the AIPC-080 Azure metadata package-surface slice. |
| 2026-05-18 | `cargo nextest run -p siumai-provider-azure --all-features azure_responses_metadata_envelopes_use_azure_namespace --no-fail-fast` | Passed | Provider metadata guard passed, proving Azure Responses metadata envelope types serialize under `azure` and not generic `openai`. |
| 2026-05-18 | `cargo nextest run -p siumai --features azure public_surface_azure_provider_ext_compiles --test public_surface_imports_test --no-fail-fast` | Passed | Public-surface compile guard passed for Azure settings, OpenAI option aliases, and Azure Responses metadata envelope exports. |
| 2026-05-18 | `cargo nextest run -p siumai-provider-azure --all-features azure_metadata azure_responses_metadata --no-fail-fast` | Passed | Focused Azure provider metadata tests passed; filter selected the new Azure Responses metadata envelope guard. |
| 2026-05-18 | `cargo nextest run -p siumai --features azure azure_siumai_provider_config_stream_custom_events_keep_raw_provider_metadata azure_registry_stream_end_metadata_match_config_path azure_registry_response_metadata_match_config_path azure_provider_metadata_key_openai_match_across_siumai_provider_config_registry --test provider_public_path_parity_test --no-fail-fast` | Passed | 4 Azure public-path parity tests passed, proving default stream/end/response metadata uses `azure` while explicit override paths consistently use `openai`. |
| 2026-05-18 | `cargo fmt -p siumai-protocol-openai -p siumai-provider-openai -p siumai -- --check` | Passed | Formatting gate for the AIPC-080 OpenAI metadata package-surface slice. |
| 2026-05-18 | `cargo nextest run -p siumai-protocol-openai --all-features openai_responses_metadata_envelopes_use_openai_namespace --no-fail-fast` | Passed | Provider metadata guard passed, proving OpenAI Responses metadata envelope types serialize under `openai` and not another provider namespace. |
| 2026-05-18 | `cargo nextest run -p siumai --features openai public_surface_openai_provider_ext_compiles --test public_surface_imports_test --no-fail-fast` | Passed | Public-surface compile guard passed for OpenAI settings/options/resources and OpenAI Responses metadata envelope exports. |
