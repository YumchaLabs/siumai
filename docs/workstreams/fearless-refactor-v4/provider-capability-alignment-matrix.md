...
| Google Vertex | Done | Done | Done | Deferred | Done | Intentionally request/options-focused on the public surface: top-level builder/provider/config parity now also locks Stable `response_format` / `tool_choice` plus provider-owned `thinkingConfig` on the final Vertex `generateContent` request body, lower builder/config/registry contract now also pins that same `response_format` / `tool_choice` plus `thinkingConfig` / `structuredOutputs` convergence directly, stream public-path plus lower-contract parity now also lock those Stable request options on `:streamGenerateContent`, provider-local `GoogleVertexClient::chat_stream_request(...)` regressions now pin `Accept: text/event-stream` plus the final stream transport body directly, mixed-registry `ProviderBuildOverrides` now explicitly cover both `generateContent` and `streamGenerateContent`, and stream public-path parity also pins `reasoning-start` / `reasoning-delta` `providerMetadata.vertex.thoughtSignature` plus final `provider_metadata["vertex"]` roots without promoting a wider typed metadata surface. |
...
- Rich native packages (`openai`, `anthropic`, `gemini` / `google`) are now in maintenance mode on
  the current public contract, while `google_vertex` remains intentionally options/tools-focused and
  `azure` remains intentionally raw/deferred on hosted metadata typing.
