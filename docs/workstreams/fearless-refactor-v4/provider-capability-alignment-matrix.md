...
| Cohere | Partial | Partial | Partial | Deferred | Done | Superseded by the newer unified-provider workstream: canonical `cohere` is now a native unified `/v2` surface for chat/embedding/rerank, public-path parity is closed for that builder/provider/config/registry story, and the remaining work is finer AI SDK parity audit for option/metadata details rather than preserving a rerank-only boundary. |
| TogetherAI | Partial | Partial | Partial | Deferred | Done | Superseded by the newer unified-provider workstream: canonical `togetherai` is now the unified public provider surface, combining shared OpenAI-compatible chat/completion/embedding/speech/transcription with provider-owned image and rerank. The remaining work is finer parity maintenance rather than a rerank-only package boundary. |
...
- Treat Cohere / TogetherAI as unified-provider stories with architecture-level public-path parity
  already closed; only reopen them for finer typed-option / metadata drift or a genuinely new
  capability surface.
