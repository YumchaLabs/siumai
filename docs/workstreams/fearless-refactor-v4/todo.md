...
- [~] Update build context flow to remain shared across all family model construction.
  ...
  - Gemini now also exposes `GeminiContentPartExt` for part-level metadata, so Google fixture assertions for text/reasoning/tool-call `thoughtSignature` no longer reopen raw nested provider maps by hand; `google-vertex` keeps its own raw namespace-key assertions in the dedicated Vertex fixture tests because `provider_ext::google_vertex` remains intentionally focused and does not grow a typed metadata surface.
...
- [~] Audit builder-only fluent options and add config-first equivalents for major providers.
  ...
  - OpenAI, Anthropic, Gemini, and OpenAI-compatible config-first ergonomics are now partially converged; remaining audit focuses on secondary providers and wrapper-specific options.
...
- [~] Wire the OpenAI-compatible runtime to an audio-capable base.
...
- [~] Normalize provider package tiers and promotion rules using `provider-package-alignment.md`, so native packages, focused packages, and OpenAI-compatible vendor presets stop drifting toward one-size-fits-all symmetry.
  - Vertex audit result: keep `provider_ext::google_vertex` options/tools-focused for now; do not add typed `metadata` / `resources` until Vertex-specific public semantics clearly diverge from the shared Google/Gemini response story.
  - That boundary is now re-validated by compile/test work too: Vertex fixture and public-surface checks continue to pass without promoting `provider_ext::google_vertex` into a typed metadata surface, while Google/Gemini typed metadata keeps carrying the shared response-story escape hatch.
