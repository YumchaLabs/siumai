...
- [x] Finalize shared model metadata trait.

- [x] Add a minimal `LanguageModel` trait spike on top of the text family.

- [x] Finalize `LanguageModel` trait.

- [x] Finalize `EmbeddingModel` trait.

- [x] Finalize `ImageModel` trait.

- [x] Finalize `RerankingModel` trait.

- [x] Finalize `SpeechModel` trait.
...
- [x] Finalize `TranscriptionModel` trait.
...
- [x] Add no-network unit tests for all trait adapters.
...
- [x] Redesign `ProviderFactory` to return family model objects instead of generic clients.

- [x] Update build context flow to remain shared across all family model construction.
...
- [x] Redesign `LanguageModelHandle` to implement the final language model trait directly.

- [x] Redesign `EmbeddingModelHandle` to implement the final embedding model trait directly.

- [x] Redesign `ImageModelHandle` to implement the final image model trait directly.

- [x] Redesign `SpeechModelHandle` to implement the final speech model trait directly.
...
- [x] Redesign `TranscriptionModelHandle` to implement the final transcription model trait directly.
...
- [x] Redesign `RerankingModelHandle` to implement the final reranking model trait directly.
...
- [x] Make provider builders emit those config structs before construction.
...
- [x] Ensure no feature can be configured only through builder APIs.
...
- [x] Audit builder-only fluent options and add config-first equivalents for major providers.
...
- [x] Normalize provider package tiers and promotion rules using `provider-package-alignment.md`, so native packages, focused packages, and OpenAI-compatible vendor presets stop drifting toward one-size-fits-all symmetry.
  ...
  - `examples/04-provider-specific/openai-compatible/README.md` now also orders examples by contract instead of provider popularity: typed compat vendor views (`openrouter`, `perplexity`) come before compat preset stories (`moonshot`, `siliconflow`), and builder-only runnable examples have been removed from the default learning path.
  - The remaining Moonshot compat examples now also carry the same package-tier note style as the newer vendor-view examples, so `moonshot-basic.rs`, `moonshot-long-context.rs`, and `moonshot-tools.rs` no longer rely on directory context alone to communicate “compat preset”.
  ...
  - Google package-surface audit now also has a dedicated workstream under `docs/workstreams/google-package-surface-alignment/`, and the stable `provider_ext::google` facade now exposes the main audited `@ai-sdk/google` typed names (`GoogleLanguageModelOptions`, `GoogleEmbeddingModelOptions`, `GoogleVideoModelOptions`, `GoogleVideoModelId`, `GoogleFilesUploadOptions`, `GoogleProviderMetadata`, `GoogleErrorData`) plus Google-branded request/upload helpers that lower onto the real provider-owned Gemini runtime for `serviceTier`, `streamFunctionCallArguments`, embedding `outputDimensionality` / `taskType` / positional multimodal `content`, and video `negativePrompt` / `personGeneration` / `referenceImages`; the current Rust task-based video helper still keeps `pollIntervalMs` / `pollTimeoutMs` as an intentional public-but-deferred boundary rather than pretending to own AI SDK-style internal polling.
  - Compat vendor-view data structures are now also closer to the audited AI SDK package indices: `provider_ext::openai_compatible` exports generic `OpenAiCompatible{Chat,Completion,Embedding}ModelId`, generic typed option structs plus deprecated migration aliases for the shared `openaiCompatible` namespace, `OpenAiCompatibleErrorData`, and `with_openai_compatible_options(...)`; `provider_ext::deepinfra` re-exports `DeepInfraErrorData`, `provider_ext::fireworks` re-exports `FireworksErrorData` plus `FireworksEmbeddingModelId` / `FireworksImageModelId`, and `provider_ext::moonshotai` re-exports `MoonshotAIChatModelId`. The same public compile guards now pin those names directly on the top-level facade while Rust intentionally keeps the model ids as stable `String` aliases instead of widening curated model constants into frozen enums, and the JS generic provider-function/settings exports remain intentionally represented by the existing Rust `OpenAiCompatibleBuilder` / `OpenAiCompatibleConfig` / `OpenAiCompatibleClient` story instead of a fake callable provider type. That shared generic helper lane now also covers chat/completion/embedding request families, and the generic chat-option lane has a top-level no-network parity anchor on the real OpenRouter builder/provider/config/registry story, so the shared `providerOptions.openaiCompatible` namespace is no longer proven only by lower compat-runtime tests.
  - xAI package-surface alignment now also closes the remaining public `providerOptions.xai` naming gap instead of stopping at upload/video parity: `XaiChatOptions`, `XaiResponsesOptions`, `XaiSearchParameters`, and search-source structs now serialize the audited AI SDK-facing camelCase fields while still accepting snake_case compatibility aliases, `XaiVideoOptions` now covers `mode` / `referenceImageUrls`, and no-network builder/provider/config/registry guards still pin both `extend-video -> /videos/extensions` and `reference-to-video -> /videos/generations + reference_images[]` on the provider-owned xAI path.
  ...
  - `togetherai` embedding now also has public-path parity anchors on both the top-level and registry embedding-handle paths for the same OpenAI-compatible request-level options, locking that `dimensions`, `encoding_format`, and `user` survive `Siumai::builder().openai().togetherai_openai_compatible()`, `Provider::openai().togetherai_openai_compatible()`, config-first `OpenAiCompatibleClient`, and `registry.embedding_model("togetherai:...")` construction consistently.
  ...
  - Embedding gap audit result is now closed for the currently enrolled catalog: every advertised embedding vendor is either covered by a parity anchor or an explicit boundary decision, and Cohere has moved out of the compat-only bucket because canonical `cohere` now owns the native `/v2/embed` public story directly.
  - Cohere embedding parity is now explicit and guard-tested on the native `/v2` package story: `provider_ext::cohere` and the top-level builder/provider/config paths all route through the same native embedding lane, while the legacy compat `openai().compatible("cohere")` path remains opt-in instead of being the primary public contract.
...
- [x] Extend the OpenAI-compatible base beyond text/embedding into speech/transcription family foundations.
  ...
  - Built-in vendor capability enrollment and contract validation are now in place for `together` and `siliconflow`; the rest of the compat catalog is explicitly deferred to provider-by-provider evidence, not a V4 core blocker.
...
- [x] Migrate Cohere.
  ...
  - `siumai-registry` now also keeps the shared provider-inference helper available under standalone `cohere` / `togetherai` builds, fixing the same cfg boundary class that previously affected `bedrock` when focused provider features compiled without unrelated text-provider features.
  - Cohere’s unified-provider contract is now explicit as well: factory/catalog tests lock it to `chat + streaming + tools + embedding + rerank`, no-network public-path tests cover the positive builder/provider/config story, and missing-model failures stay explicit instead of silently inheriting a provider-wide default.
  - The registry factory `language_model_with_ctx` entry is now pinned to that same unified story too: Cohere now materializes a real native chat handle on the generic language-model path, and contract tests lock the same provider-owned routing instead of a compatibility fallback.
...
- [x] Migrate TogetherAI.
  ...
  - TogetherAI’s unified-provider boundary is now explicit too: factory/catalog tests lock the mixed compat text/audio + provider-owned image/rerank capability set, and no-network public-path tests now cover the intended positive builder/provider/config construction instead of a rerank-only rejection story.
  - The canonical TogetherAI image lane now also matches the audited AI SDK package: generation and edit both use provider-owned `/images/generations`, edits map the first input image to `image_url`, mask edits are rejected up front, and public `TogetherAiImageOptions` / request helpers now cover the typed image-option lane without raw JSON mutation.
...
- [x] Migrate Bedrock.
  ...
  - `siumai/build.rs` now also counts `cohere`, `togetherai`, and `bedrock` as valid provider features, so standalone focused-provider / Bedrock-only builds no longer fail the top-level “at least one provider enabled” guard before public-path tests even start.
  - `siumai-registry` single-feature compile sweep is now green for the current native provider set (`openai`, `azure`, `anthropic`, `google`, `google-vertex`, `ollama`, `xai`, `groq`, `deepseek`, `minimaxi`, `cohere`, `togetherai`, `bedrock`), and the built-in registry registration checks for `cohere`, `togetherai`, and `bedrock` are now all explicitly validated in feature-minimal mode.
  ...
  - Amazon Bedrock now also has matching top-level public registry anchors for that same split boundary: `registry.reranking_model("bedrock:...")` matches config-first `/rerank` request shaping, `registry.embedding_model("bedrock:...")` now also matches config-first `/model/{id}/invoke` embedding request shaping, and `registry.image_model("bedrock:...")` still fails before transport on the real public registry path.
...
- [x] Migrate MiniMaxi.
  ...
  - The same MiniMaxi stream metadata normalization now also reaches typed stream parts, not just final `StreamEnd`: finish-part `provider_metadata` is rekeyed to the public `minimaxi` root before the stable stream lane is exposed.
...
- [x] Migrate DeepSeek.
  ...
  - Provider-owned `DeepSeekConfig` / `DeepSeekClient` entry points, registry-native text-family routing, deepseek-only feature wiring, unified builder routing, provider-owned typed request options (`DeepSeekOptions` / `DeepSeekChatRequestExt`), provider-owned typed response metadata (`DeepSeekMetadata` / `DeepSeekChatResponseExt`), a provider-owned `DeepSeekSpec` normalization layer for chat request options, and a first-class `ProviderType::DeepSeek` are now in place; registry and unified builder paths now also materialize the provider-owned `DeepSeekClient` instead of leaking the generic OpenAI-compatible runtime, while deeper native-provider expansion is now provider-depth follow-on work rather than unfinished V4 migration.
  - DeepSeek public data-structure parity is now closer to the audited AI SDK package too: the provider crate now exposes AI SDK-style `DeepSeekLanguageModelOptions` plus deprecated `DeepSeekChatOptions`, the public facade now exposes curated `provider_ext::deepseek::{chat, model_sets}` constants for the stable `deepseek-chat` / `deepseek-reasoner` subset, and provider catalog output now reuses the same `models::ALL_CHAT` source instead of retyping that subset in a second list.
  ...
  - DeepSeek top-level public metadata parity now also respects that same provider-owned boundary:
    public-path guards assert response-level `id` / `model` on `ChatResponse` itself while
    `DeepSeekMetadata` stays limited to provider-owned `logprobs` / `sources` / `extra`, so the
    wrapper no longer reintroduces Groq/xAI-style metadata assumptions onto the DeepSeek namespace.
  ...
  - Vertex Imagen mixed-registry override parity now also covers the full typed `VertexImagenOptions`
    generation subset on the public handle path, so `registry.image_model("vertex:imagen-...")`
    no longer proves only auth/base-url precedence while silently skipping provider-owned image
...
- [-] Use `provider-capability-alignment-matrix.md` as the canonical backlog driver for the next provider capability alignment pass.
  ...
  - One small historical example-debt pocket is now also closed: the `06-extensibility` examples have been updated to the current `HttpChatExecutor` shape, so `cargo test -p siumai --no-run` no longer stops on stale non-optional transformer fields from those custom-provider samples.
...
- [x] Secondary provider cleanup notes:
...
## Track J - OpenAI-compatible internal boundary cleanup

- [x] Split the OpenAI-compatible provider model catalog by provider family.
