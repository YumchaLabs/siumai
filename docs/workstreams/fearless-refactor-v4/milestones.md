...
- Cohere and TogetherAI now also have explicit provider contracts on the public path: canonical `cohere` is a unified native `/v2` chat/embedding/rerank surface, canonical `togetherai` is the unified public provider story that combines shared compat chat/completion/embedding/speech/transcription with provider-owned image/rerank, and registry/catalog tests plus no-network public-path coverage now lock those intended capability sets directly.
- The same provider-contract closure is now pinned on the registry factory’s generic `language_model_with_ctx` entry as well: Cohere and TogetherAI now keep the positive text-handle path where their canonical public story supports it, instead of depending on fake rerank-only text fallbacks or rejecting real unified provider identities.
...
- Bedrock’s earlier native fail-fast boundary is now fully superseded: after the first chat/streaming/tools/rerank closure, later Bedrock embedding and image audits promoted both `embedding` and `image_generation` to real provider-owned `/model/{id}/invoke` family paths, so the current capability story is no longer text+rerank-only.
...
- Cohere and TogetherAI config-first ergonomics now also mirror their focused provider builders’ common HTTP surface directly on `CohereConfig` / `TogetherAiConfig`: timeout / connect-timeout and single-interceptor composition now have canonical config-first helpers, and package-local parity tests pin `CohereBuilder::into_config()` / `TogetherAiBuilder::into_config()` against those same config-first surfaces.
...
- Google Vertex batch embedding parity now also reaches the top-level family helper surface: `GoogleVertexClient` coalesces homogeneous `BatchEmbeddingRequest` values into one native embedding `:predict` call with multiple `instances` when that transformation is lossless, preserves the sequential fallback for mixed-shape batches, and public no-network coverage now locks `embedding::embed_many(...)` across builder/provider/config-first plus the registry embedding handle.
...
- Gemini batch embedding parity now also reaches the top-level family helper surface: `GeminiClient` coalesces homogeneous `BatchEmbeddingRequest` values into one native `:batchEmbedContents` call when that transformation is lossless, preserves the sequential fallback for mixed-shape batches, and public no-network coverage now locks `embedding::embed_many(...)` across builder/provider/config-first plus the registry embedding handle.
...
- `siumai-registry` now also compiles cleanly under standalone `cohere` / `togetherai` feature builds with the shared provider-inference helper kept in scope, removing another feature-boundary regression in `SiumaiBuilder`.
...
- Amazon Bedrock migration now also has a provider-owned config/client/builder path with split runtime vs agent-runtime endpoint ownership, registry construction now materializes the provider-owned typed client directly, and top-level public-path parity now locks chat, chat-stream, rerank, and image request shaping across siumai/provider/config-first entry points.
- Amazon Bedrock now also has matching builder/config/registry contract anchors for chat, chat-stream, rerank, and image request shaping, locking runtime vs agent-runtime endpoint routing, Bearer auth propagation, and Bedrock-specific request-body passthrough on the provider-factory path itself.
- Amazon Bedrock now also has the matching top-level public registry boundary: `registry.reranking_model("bedrock:...")`, `registry.embedding_model("bedrock:...")`, and `registry.image_model("bedrock:...")` all match config-first request shaping on `/rerank` or `/model/{id}/invoke`, so the public image family no longer trails the lower contract coverage.
...
- `siumai/build.rs` now also recognizes `cohere`, `togetherai`, and `bedrock` as first-class provider features, so standalone focused-provider / Bedrock-only public/test builds no longer trip the crate-level provider guard before the actual provider-boundary checks run.
...
- Provider feature alignment work is tracked in `provider-feature-matrix.md`.
  ...
  - The generic registry text guard is now provider-agnostic instead of compat-specific: non-chat providers like Jina and VoyageAI still reject `registry.language_model("provider:model")` construction, while unified providers such as Cohere and TogetherAI keep the positive text-handle path the public architecture now advertises.
...
- Cohere and TogetherAI public-path parity are now also explicitly closed in the capability matrix: canonical `cohere` is the native unified `/v2` chat/embedding/rerank story, canonical `togetherai` is the unified public provider story with provider-owned image/rerank plus shared compat chat/completion/embedding/speech/transcription, and their no-network builder/provider/config-first anchors plus public-surface compile guards confirm there is no hidden top-level architecture gap left to chase in this pass.
...
- This historical milestone is now superseded by the newer Cohere unified-provider workstream:
  canonical `cohere` now owns native `/v2` chat/embedding/rerank on the public path.
- The older compat-only Cohere embedding boundary is no longer the canonical story. The
  OpenAI-compatible Cohere preset remains opt-in compatibility only.
...
See also: `docs/releasing.md` and `CHANGELOG.md`.
