...
- Gemini now also exposes `GeminiContentPartExt` for part-level metadata reads, so Google thought-signature fixture checks for text/reasoning/tool-call parts no longer depend on raw nested provider-map traversal; the dedicated `google-vertex` fixture still keeps explicit raw namespace-key assertions because `provider_ext::google_vertex` remains intentionally options/tools-focused rather than growing a typed metadata surface.
- The complementary package-tier boundary is also now enforced: `provider_ext::google_vertex` remains options/tools-focused and is explicitly *not* promoted into a typed metadata surface yet, while Vertex fixture/public-surface tests stay green on that narrower contract.
...
- Focused gap audit now also locks that `provider_ext::google_vertex` stays options/tools-led and `provider_ext::bedrock` stays request-helper-led until either provider accumulates clearly provider-owned public metadata/resources worth stabilizing.
...
- Top-level compat preset guards now also lock the shared-registry construction path for `Provider::openai().{siliconflow,together,fireworks}()`, together with the intended audio capability split: `siliconflow` and `together` remain speech+transcription capable, while `fireworks` remains transcription-only on audio.
