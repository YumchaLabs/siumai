...
| Google Vertex | No | No | No | Yes | Intentionally remains options/tools-focused; no typed metadata surface is promoted for V4, and top-level builder/provider/config-first stream parity now explicitly locks `reasoning-start` / `reasoning-delta` `providerMetadata.vertex.thoughtSignature` plus final `provider_metadata["vertex"]` roots on that raw boundary. |
...
- `provider_ext::google_vertex` remains request/options-focused.
