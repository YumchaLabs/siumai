...
## [0.11.0-beta.6](https://github.com/YumchaLabs/siumai/compare/siumai-registry-v0.11.0-beta.5...siumai-registry-v0.11.0-beta.6) - 2026-03-25

### Added

- *(v4)* align provider parity and typed metadata surfaces
- *(azure)* align unified builder url and metadata parity
- *(registry)* align provider factories and handle routing
- *(registry)* forward image and rerank capabilities
...
- *(ollama)* restore request option parity across registry paths
- *(embedding)* delegate wrapper and batch family paths
- *(embedding)* preserve request config on family model paths
- *(registry)* forward full ChatRequest in language model handle
...
- *(registry)* deprecate Siumai::builder
