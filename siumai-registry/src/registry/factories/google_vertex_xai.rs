...
//!
//! AI SDK exposes `@ai-sdk/google-vertex/xai` as its own provider surface backed by
...
fn google_vertex_xai_capabilities() -> ProviderCapabilities {
    crate::native_provider_metadata::native_providers_metadata()
        .into_iter()
        .find(|meta| meta.id == ids::GOOGLE_VERTEX_XAI)
...
> {
    ...
    #[cfg(not(feature = "gcp"))]
    {
        Err(LlmError::ConfigurationError(
            "Google Vertex xAI requires an Authorization header, explicit api_key bearer token, or a Google token provider".to_string(),
        ))
    }
}
...
/// Google Vertex xAI provider factory.
#[cfg(feature = "google-vertex")]
pub struct GoogleVertexXaiProviderFactory;

#[cfg(feature = "google-vertex")]
#[async_trait::async_trait]
impl ProviderFactory for GoogleVertexXaiProviderFactory {
    fn capabilities(&self) -> ProviderCapabilities {
        google_vertex_xai_capabilities()
    }
    ...
    fn provider_id(&self) -> Cow<'static, str> {
        Cow::Borrowed(ids::GOOGLE_VERTEX_XAI)
    }
}
