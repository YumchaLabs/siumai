...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let media_type = if let Ok(media_type) = std::env::var("TOGETHER_AUDIO_MEDIA_TYPE") {
        media_type
    } else if let Some(media_type) = infer_audio_media_type(&audio_file) {
        media_type.to_string()
    } else {
        return Err(
            "Could not infer TOGETHER audio media type; set TOGETHER_AUDIO_MEDIA_TYPE".into(),
        );
    };
    let request = SttRequest::from_audio(audio_bytes, media_type).with_model(TOGETHER_STT_MODEL);
