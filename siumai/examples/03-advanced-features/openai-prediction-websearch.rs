...
//! 2) Web search options (context size + user location)
//!
//! Learn more:
...
async fn web_search_context_size(client: &Siumai) -> Result<(), Box<dyn std::error::Error>> {
    ...
    .with_openai_options(
        OpenAiOptions::new().with_web_search_options(
            OpenAiWebSearchOptions::new().with_context_size("high"),
        ),
    );
...
async fn web_search_location(client: &Siumai) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let req = ChatRequest::new(vec![user!("What's the weather like today?")]).with_openai_options(
        OpenAiOptions::new().with_web_search_options(
...
async fn combined(client: &Siumai) -> Result<(), Box<dyn std::error::Error>> {
    println!("4) Combined: prediction + web search\n");
