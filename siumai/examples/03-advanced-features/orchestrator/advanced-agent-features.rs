...
//! Run with:
//! ```bash
//! cargo run --example advanced-agent-features --features openai
//! ```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let agent3 = ToolLoopAgent::new(
        client.clone(),
        tools.clone(),
        vec![Box::new(has_tool_result())], // Stop after first tool result
    )
    .with_system("You are a helpful assistant.");
    ...
    let agent4 = ToolLoopAgent::new(
        client.clone(),
        tools.clone(),
        vec![Box::new(has_no_tool_calls())], // Stop when model doesn't call tools
    )
    .with_system("You are a helpful assistant.");
