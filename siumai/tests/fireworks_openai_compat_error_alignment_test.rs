...
#[test]
fn fireworks_openai_compatible_error_envelope_is_parsed() {
    let body = fixture_text(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("fireworks")
            .join("errors")
            .join("fireworks-authentication-error.1.json"),
    );

    let err = classify_openai_compatible_http_error("fireworks", 401, &body)
