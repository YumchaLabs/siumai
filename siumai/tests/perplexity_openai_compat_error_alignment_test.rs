...
#[test]
fn perplexity_openai_compatible_error_envelope_is_parsed() {
    let body = fixture_text(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("perplexity")
            .join("errors")
            .join("perplexity-authentication-error.1.json"),
    );

    let err = classify_openai_compatible_http_error("perplexity", 401, &body)
