...
#[test]
fn openai_responses_code_interpreter_stream_emits_vercel_aligned_events() {
    ...
    let has_document_sources = event_payloads_by_type(&events, "source")
        .iter()
        .any(|value| value.get("sourceType") == Some(&serde_json::json!("document")));
    assert!(has_document_sources, "expected document sources");
}
