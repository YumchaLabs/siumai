...
#[test]
fn vertex_generate_content_request_bridge_roundtrip_fixture_exact_cases_match() {
    let exact_cases = [
...
#[test]
fn vertex_generate_content_request_bridge_documents_provider_tool_projection() {
    let projected_cases = [
        ("google-google-search.1", "googleSearch"),
        ("google-code-execution.1", "codeExecution"),
    ];

    for (case, expected_key) in projected_cases {
        let bridged = bridge_request(case);
        assert!(!bridged.is_rejected());

        let raw_value = bridged.value.expect("bridged request body");
        assert!(
            raw_value["tools"][0].get(expected_key).is_some(),
            "fixture case {} expected projected `{expected_key}` tool before normalization: {raw_value:?}",
            fixtures_dir().join(case).display()
        );
        ...
        assert_eq!(
            ...
            "fixture case {} should document model uplift for provider-hosted tools",
            fixtures_dir().join(case).display()
        );
        assert!(
            value["tools"]
            ...
            "fixture case {} expected one projected provider tool: {value:?}",
            fixtures_dir().join(case).display()
        );
    }
}
