...
fn make_openai_converter() -> OpenAiCompatibleEventConverter {
    ...
    impl ProviderAdapter for Adapter {
        ...
        fn compatibility(&self) -> siumai::providers::openai_compatible::adapter::ProviderCompatibility {
...
async fn chat_completions_role_content_finish_reason_fixture() {
    ...
    // Assert: at least one ContentDelta and a final StreamEnd
    assert!(events.iter().any(|e| matches!(e, ChatStreamEvent::ContentDelta { .. })), "expect content delta");
    assert!(events.iter().any(|e| matches!(e, ChatStreamEvent::StreamEnd { .. })), "expect stream end");
}
...
async fn chat_completions_tool_calls_arguments_usage_fixture() {
    ...
    let saw_tool_call = events.iter().any(|e| matches!(
        e,
    ...
    assert!(saw_tool_call, "expect initial tool call delta with id and name");
    assert_eq!(combined_args, "{\"q\":\"rust\"}");
    assert!(saw_usage, "expect usage update event");
    assert!(saw_end, "expect final stream end");
}
...
async fn chat_completions_multiple_tool_calls_at_least_one_fixture() {
    ...
    assert!(events.iter().any(|e| matches!(e, ChatStreamEvent::ToolCallDelta { .. })), "expect at least one tool call delta");
}
