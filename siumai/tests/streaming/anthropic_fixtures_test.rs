...
async fn anthropic_thinking_and_text_fixture() {
    let bytes =
        support::load_sse_fixture_as_bytes("tests/fixtures/anthropic/thinking_and_text.sse")
            .expect("load fixture");
...
async fn anthropic_error_event_fixture() {
    ...
    let bytes = support::load_sse_fixture_as_bytes("tests/fixtures/anthropic/error_event.sse")
        .expect("load fixture");
    let converter = make_anthropic_converter();
    let events = support::collect_sse_events(bytes, converter).await;
    assert!(
        events.iter().any(
            |e| matches!(e, ChatStreamEvent::Error { error } if error.contains("Invalid auth"))
        )
    );
}
...
async fn anthropic_partial_without_message_stop_fixture() {
    // Stream ended without message_stop event - should emit StreamEnd with Unknown reason
    let bytes = support::load_sse_fixture_as_bytes(
    ...
    // Should have StreamEnd with Unknown reason (model did not transmit finish reason)
    let stream_end = events
        ...
        .expect("expect StreamEnd");

    assert_eq!(
        stream_end.finish_reason,
        Some(siumai::types::FinishReason::Unknown),
        "expect Unknown finish_reason for stream without message_stop"
    );
}
...
async fn anthropic_refusal_stop_reason_maps_to_content_filter() {
    let bytes =
        support::load_sse_fixture_as_bytes("tests/fixtures/anthropic/refusal_stop_reason.sse")
            .expect("load fixture");
