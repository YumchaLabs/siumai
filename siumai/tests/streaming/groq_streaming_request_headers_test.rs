//! Verify Groq streaming request headers at call-site
...
struct Seen { accept_is_sse: bool, accept_encoding_identity: bool, stream_true: bool }
...
async fn handler(req: Request, state: Arc<Mutex<Seen>>) -> Response {
    ...
    if let Ok(json) = serde_json::from_slice::<serde_json::Value>(&body_bytes) {
        seen.stream_true = json.get("stream").and_then(|v| v.as_bool()) == Some(true);
    }
...
#[tokio::test]
async fn groq_streaming_headers_default_identity() {
    let state = Arc::new(Mutex::new(Seen::default()));
    let app = { let s=state.clone(); Router::new().route("/chat/completions", post(move |req| handler(req, s.clone()))) };
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tokio::spawn(async move { axum::serve(listener, app).await.unwrap() });

    let base = format!("http://{}:{}", addr.ip(), addr.port());
    let client = Provider::groq().api_key("test").base_url(base).model("llama-3.1-8b-instant").build().await.expect("client");
    ...
    assert!(seen.stream_true);
    drop(server);
}

#[tokio::test]
async fn groq_streaming_headers_disable_identity() {
    let state = Arc::new(Mutex::new(Seen::default()));
    let app = { let s=state.clone(); Router::new().route("/chat/completions", post(move |req| handler(req, s.clone()))) };
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    ...
    let base = format!("http://{}:{}", addr.ip(), addr.port());
    let client = Provider::groq().api_key("test").base_url(base).stream_disable_compression(false).model("llama-3.1-8b-instant").build().await.expect("client");
    let mut stream = client.chat_stream(vec![user!("hi")], None).await.expect("stream ok");
    while let Some(ev) = stream.next().await { if matches!(ev.unwrap(), ChatStreamEvent::StreamEnd{..}) { break; } }
    let seen = state.lock().await.clone();
    assert!(seen.accept_is_sse);
