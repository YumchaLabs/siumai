...
fn run_case(root: &Path) {
    ...
    if expected_warnings_path.exists() {
        use siumai_provider_openai::execution::LanguageModelMiddleware;
        use siumai_provider_openai::providers::openai::OpenAiResponsesInputWarningsMiddleware;

        let expected_warnings: Vec<Warning> = read_json(expected_warnings_path);
        let mw = OpenAiResponsesInputWarningsMiddleware::new();
...
#[test]
fn openai_responses_input_fixtures_match() {
