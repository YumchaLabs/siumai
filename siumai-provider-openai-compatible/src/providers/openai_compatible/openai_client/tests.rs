...
use crate::provider_options::{
    AlibabaChatOptions, FireworksChatOptions, FireworksReasoningHistory, FireworksThinkingConfig,
...
async fn chat_request_runtime_mistral_maps_model_length_finish_reason() {
    ...
    let cfg =
        OpenAiCompatibleConfig::new("mistral", "test-key", "https://api.mistral.ai/v1", adapter)
...
async fn chat_stream_request_runtime_omits_stream_options_by_default_for_compat_provider() {
    ...
    let cfg =
        OpenAiCompatibleConfig::new("deepseek", "test-key", "https://api.deepseek.com", adapter)
            .with_model("deepseek-chat")
            .with_http_transport(Arc::new(transport.clone()));
