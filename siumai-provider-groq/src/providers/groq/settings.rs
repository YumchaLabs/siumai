...
use super::{GroqBuilder, GroqConfig};

/// Package-level provider settings aligned with
/// `repo-ref/ai/packages/groq/src/groq-provider.ts`.
///
/// Unlike `GroqConfig`, this carrier intentionally does not require a model id.
...
impl GroqProviderSettings {
    ...
    pub fn into_builder(self) -> GroqBuilder {
        let mut builder = GroqBuilder::new(BuilderBase::default());
    ...
    pub fn into_builder_for_model<S: Into<String>>(self, model: S) -> GroqBuilder {
        self.into_builder().model(model)
    }

    pub fn into_config_for_model<S: Into<String>>(self, model: S) -> Result<GroqConfig, LlmError> {
