//! Curated TogetherAI model constants aligned with the audited AI SDK package subset.
...
mod tests {
    ...
    fn curated_defaults_match_current_runtime_defaults() {
        assert_eq!(CHAT, chat::META_LLAMA_3_1_8B_INSTRUCT_TURBO);
        assert_eq!(EMBEDDING, embedding::M2_BERT_80M_8K_RETRIEVAL);
        assert_eq!(IMAGE, image::FLUX_1_SCHNELL);
        assert_eq!(RERANK, rerank::LLAMA_RANK_V1);
    }

    #[test]
    fn curated_lists_include_primary_defaults() {
        assert!(ALL_CHAT.contains(&CHAT));
        assert!(ALL_COMPLETION.contains(&COMPLETION));
