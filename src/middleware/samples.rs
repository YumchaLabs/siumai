...
impl SimulateStreamingMiddleware {
    pub fn new(chunk_size: usize, delay_ms: Option<u64>) -> Self {
        Self { chunk_size: chunk_size.max(1), delay_ms }
    }
}
...
mod tests {
    ...
    async fn simulate_streaming_chunks_when_no_deltas() {
        ...
        assert!(evs.iter().any(|e| matches!(e, Ok(ChatStreamEvent::StreamEnd { .. }))));
    }
    ...
    async fn simulate_streaming_passthrough_when_deltas_exist() {
        ...
        let delta_count = evs.iter().filter(|e| matches!(e, Ok(ChatStreamEvent::ContentDelta{..}))).count();
