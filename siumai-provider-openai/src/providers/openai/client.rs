...
impl OpenAiClient {
    ...
    pub fn set_forced_responses_api(
        &mut self,
        cfg: Option<crate::provider_options::openai::ResponsesApiConfig>,
    ) {
...
mod tests {
    ...
    #[test]
    fn responses_default_provider_options_map_merged_and_request_overrides() {
        let mut cfg = OpenAiConfig::new("test-key").with_model("gpt-4.1-mini");
        ...
        // Request overrides only the previousResponseId; defaults should still apply (e.g. include).
        let request =
            crate::types::ChatRequest::new(vec![crate::types::ChatMessage::user("hi").build()])
                .with_provider_option(
                    ...
                    serde_json::json!({
                        "responsesApi": {
                            "previousResponseId": "resp_override"
                        }
                    }),
                );
