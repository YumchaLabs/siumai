...
use super::{XaiBuilder, XaiConfig};

/// Package-level provider settings aligned with
/// `repo-ref/ai/packages/xai/src/xai-provider.ts`.
///
/// Unlike `XaiConfig`, this carrier intentionally does not require a model id.
...
impl XaiProviderSettings {
    ...
    pub fn into_builder(self) -> XaiBuilder {
        let mut builder = XaiBuilder::new(BuilderBase::default());
    ...
    pub fn into_builder_for_model<S: Into<String>>(self, model: S) -> XaiBuilder {
        self.into_builder().model(model)
    }

    pub fn into_config_for_model<S: Into<String>>(self, model: S) -> Result<XaiConfig, LlmError> {
